...
class Lobby
    ...
    def generate_game(args={})
        if @game_state == :genesis
            ...
            broadcast(Message.new(:generation_fail, {:reason => "Lobby is not in :genesis (#{@game_state})"}))
            false
        end
    end
    ...
    def start_game
        if @game_state == :ready
            ...
            broadcast(Message.new(:start_fail, {:reason => "Lobby is not :ready (#{@game_state})"}))
            false
        end
    end
end
